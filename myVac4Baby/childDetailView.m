...
@implementation childDetailView
...
- (void)viewDidLoad {
    ...
    app.navigateTo = @"createChild2";
    
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(receiveNotification:)
                                                 name:@"createChild2_receivseNotification"
                                               object:nil];
}

- (void) receiveNotification:(NSNotification *) notification
{
    if ([[notification name] isEqualToString:@"createChild2_receivseNotification"])
    {
...
- (IBAction)createChild:(id)sender {
    ...
    if([txtChildName.text length] == 0)
    {
        ...
        /* set txtName placeholder in color */
    ...
    {
        // if skip value != 2, pass an empty image instead
...
-(void) createSecondNotifications:(NSString*) childId ichildImageUrl:(NSString*)childImageUrl
{
    ...
    for(int i = 0; i < 10; i++)
    {
        ...
        NSLog(@"fire date (in date format) : %@", localNotification.fireDate);
