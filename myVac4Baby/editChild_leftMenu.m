...
@interface editChild_leftMenu ()

@end

@implementation editChild_leftMenu
...
- (void)viewDidLoad {
    ...
    app.navigateTo = @"childProfileLeftMenu";
    
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(receiveNotification:)
                                                 name:@"childProfileLeftMenu_receiveNotification"
                                               object:nil];
}

- (void) receiveNotification:(NSNotification *) notification
{
    if ([[notification name] isEqualToString:@"childProfileLeftMenu_receiveNotification"])
    {
...
-(void) viewWillAppear:(BOOL)animated
{
    ...
    childProfile = [[NSUserDefaults standardUserDefaults] objectForKey:@"SelectedChildToEditProfile"];
    
    NSString *s = LocalizedString(@"profile");
    
    NSString *str = [NSString stringWithFormat:@"%@'s %@", [[childProfile objectAtIndex:0] objectForKey:@"name"], s];
...
-(void) setUpView
{
    ...
    [txtChildName setText:[[childProfile objectAtIndex:0] objectForKey:@"name"]];
    ...
    [imgBaby setImageWithURL:[NSURL URLWithString:[[childProfile objectAtIndex:0] objectForKey:@"url"]] placeholderImage:[UIImage imageNamed:@"default_addaphotoofyourbaby"]];
    
    [txtDate setText:[self formatDate:[[childProfile objectAtIndex:0] objectForKey:@"dob"]]];
    ...
    NSDate *currentDate = [dateFormatter dateFromString:[[childProfile objectAtIndex:0] objectForKey:@"dob"]];
    
    [pickerView setDate:currentDate];
    
    [txtDate setInputView:pickerView];
    
    [self setGenderImg:[[childProfile objectAtIndex:0] objectForKey:@"gender"]];
    
    strDOB = [[childProfile objectAtIndex:0] objectForKey:@"dob"];
...
- (IBAction)saveInfo:(id)sender {
    ...
    [dict setValue:[[childProfile objectAtIndex:0] objectForKey:@"childId"] forKey:@"childId"];
...
- (void)connection:(NSURLConnection *)theConnection didReceiveData:(NSData *)data
{
    ...
    if([[json objectForKey:@"action"] isEqualToString:@"editChildProfile"])
    {
        if([[json objectForKey:@"result"] intValue] == 1)
        {
            ...
            if(![[[childProfile objectAtIndex:0] objectForKey:@"dob"] isEqualToString:strDOB])
            {
                [self removePreviousReminder:[[childProfile objectAtIndex:0] objectForKey:@"childId"]];
                
                // re-create 1st reminder
                [self createFirstNotifications:[[childProfile objectAtIndex:0] objectForKey:@"childId"] ichildImageUrl:[json objectForKey:@"childImageUrl"]];
                
                // re-create 2nd reminder
                [self createSecondNotifications:[[childProfile objectAtIndex:0] objectForKey:@"childId"] ichildImageUrl:[json objectForKey:@"childImageUrl"]];
                ...
                [dbNotification deleteNotificationsByChildId:[[childProfile objectAtIndex:0] objectForKey:@"childId"]];
            ...
            userProfileView *mmvc = [self.storyboard instantiateViewControllerWithIdentifier:@"userProfileVC"];
            mmvc.navigateFromSideMenuEditChild = @"didNavigateFromSideMenu";
